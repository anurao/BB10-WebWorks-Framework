...
webview =
    ...
    create: function (ready) {
        _webviewObj = window.qnx.webplatform.createWebView({processType: OUT_OF_PROCESS}, function () {
            ...
            /* Enable the permissions checking for this webview, traps any native permission based
             * on the permissions the user granted WW, we then use the AuthMan to check and suppress
             * any wekbit based HTML notification/geolocation requests
             */
            permissions.init(this);
        //    _webviewObj.onGeolocationPermissionRequest = permissions.onGeolocationPermissionRequest;
           // _webviewObj.onNotificationPermissionRequest = permissions.onNotificationPermissionRequest;
    ...
    disallowNotification : function (url) {
        _webviewObj.disallowNotification(url);
    }
};
...
webview.__defineSetter__('onNotificationPermissionRequest', function (input) {
    _webviewObj.onNotificationPermissionRequest = input;
});
