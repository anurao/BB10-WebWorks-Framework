...
function rebuildRequest(req) {
    ...
    if (finalToken && finalToken.indexOf("?") >= 0) {
        ...
        params = finalToken.split("?")[1].split("&");
        if (params) {
            args = {};  
...
function _formMessage(url, origin, body) {
...
module.exports = {
    ...
    handleRpc: function (url, body) {
        //TODO get origin
        var message = _formMessage(url, "local:///automatic/SpecRunner.htm", body);
        this.handle(message.request, message.response);
