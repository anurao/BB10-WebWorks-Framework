    ...
    function createWebworksReady() {
        function RemoteFunctionCall(functionUri) {
            ...
            this.makeAsyncCall = function (success, error) {
                window.qnx.rpc.remoteExec("client.remoteFunctionCall", [functionUri, params], function (responseText) {
                    var errored,
                        data,
                        cb,
                        response;

                    response = JSON.parse(decodeURIComponent(responseText) || "null");
                    errored = response.code < 0;
                    cb = errored ? error : success;
                    data = errored ? response.msg : response.data;
        ...
        window.webworks = {
            ...
            execAsync: function (service, action, args, success, fail) {
                execFunc(success, fail, service, action, args, false);
            },
