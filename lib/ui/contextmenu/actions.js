...
var LIB_FOLDER = "./../../../lib/",
    ...
    _tempStorage = require(LIB_FOLDER + 'temp'),
    _menuItems,
    _currentContext,
...
function invokeFile(fileUri, transferMode, callback) {

    //Build our queryTargets request
    var request = {
        action: 'bb.action.VIEW',
        uri: fileUri,
        file_transfer_mode : transferMode
    };

    window.qnx.webplatform.getApplication().invocation.invoke(request, function (params) {
        console.log("We just invoked a new viewer" + params);
...
function viewImage() {
    ...
    var fileUri,
        transferMode,
        directory = window.qnx.webplatform.getApplication().getEnv("HOME"),
        cardClosedEvent;
    ...
    // Set the proper transfer mode as read only
    transferMode = _application.invocation.FILE_TRANSFER_COPY_RO;

    if (_utils.startsWith(_currentContext.src, "local:///")) {

        fileUri = "file:/" + directory + "/../app/native/" + _currentContext.src.replace(/local:\/\/\//, '');
        invokeFile(fileUri, transferMode, function (params) {
            console.log("Invoked image viewer card");
        });

    } else {
        // Save the image to temporary storage
        _tempStorage.saveImage(_currentContext.src, function (filePath) {

            invokeFile(filePath, transferMode, cardClosedEvent = function (params) {
                window.qnx.webplatform.getApplication().invocation.addEventListener("cardClosed", function () {

                    // Remove the onCardClosed listener
                    _tempStorage.removeFile(filePath, function () {
                        window.qnx.webplatform.getApplication().invocation.removeEventListener("cardClosed", cardClosedEvent);
                    });
                });
    ...
    function onSaved(target) {
        ...
        if (target) {
            var request = {
                ...
                target_type: _application.invocation.TARGET_TYPE_MASK_VIEWER
            };
